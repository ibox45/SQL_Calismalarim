...
select ulke, count(*) from personel group by ulke;  -- herhangi birşeyi saysa o da olur burada.
...
select ulke ,sehir,count(*)   from personel group by ulke, sehir;

-- SORU07: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.

select ulke,cinsiyet,count(*), avg(yas) from personel group by ulke,cinsiyet;

-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.

select ulke,cinsiyet,count(*),avg(yas) from personel where maas>30000 group by ulke,cinsiyet;
...
select ulke,cinsiyet, count(*),avg(yas) from personel where maas>30000  group by ulke,cinsiyet order by avg(yas) desc;
...
select isim,urun_adi , sum(urun_miktari) from manav where urun_adi is not null group by isim,urun_adi order by isim desc;  -- null olmasın dedik.
...
-- select isim, urun_adi, sum(urun_miktari) as toplam  from manav where toplam >=3 group by isim,urun_adi order by toplam desc; -- hata veriyor.
-- aggregate fonksiyon kullandıysak where yerine having kullancagız.
-- select isim, urun_adi, sum(urun_miktari) as toplam from manav having toplam >=3 group by isim,urun_adi order by toplam desc;  -- bu sıralama da hata veriyor.
select isim, urun_adi, sum(urun_miktari) as toplam  from manav  group by isim,urun_adi having toplam >=3 order by toplam desc; 
...
select urun_adi, max(urun_miktari) as maksimum from manav group by urun_adi having maksimum!=min(urun_miktari) order by maksimum desc;
-- eşit degildir için (<>) bu da kullanılır (!=) bu da
-- maksimum!=min(urun_miktari)
-- maksimum<>min(urun_miktari)
