...
select marka_isim, calisan_sayisi, (select round(avg(maas),2) from calisanlar where marka_isim=isyeri) as ortalama_maas from markalar;  -- as ortalama_maas ile parantez icini isimlendirmiş oluyorum, as sözcüğü olmasa da olur
-- round kullanınca yuvarlama yaptı. ,2 yazarak da virgulden sonra 2 basamak göster demiş olduk
...
select marka_isim, calisan_sayisi, (select max(maas) from calisanlar where marka_isim=isyeri) from markalar;
select marka_isim, calisan_sayisi, (select min(maas) from calisanlar where marka_isim=isyeri)  from markalar;
...
select marka_id, marka_isim , (select count(sehir) from calisanlar where marka_isim=isyeri) from markalar;

-- eger aynı sehir ismi 2 yada daha fazla gecıorsa yanı tekrar edenlerı saymasını ıstemıyorsak distinct kullanıyoruz 
select marka_id, marka_isim , (select count(distinct sehir) from calisanlar where marka_isim=isyeri) from markalar;
